...
@Component({
  ...
  template: `
    <div class="custom-select-container" (click)="toggleDropdown()">
      <button type="button" class="select-display">
        ...
        <svg class="arrow-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="48" d="M112 184l144 144 144-144" /></svg>
      </button>
      @if (isOpen) {
  ...
  styles: [`
    ...
    .custom-select-container { cursor: pointer; }
    .select-display {
      ...
      border-radius: 4px;
      padding: 4px 8px; font-size: 1rem; text-align: left;
    }
    .arrow-icon { width: 12px; height: 12px; margin-left: 8px; }
    .options-panel {
    ...
    .options-panel ul { list-style: none; padding: 4px; margin: 0; }
    .options-panel li { padding: 8px 12px; border-radius: 4px; cursor: pointer; }
    .options-panel li:hover { background-color: var(--datepicker-hover-background, #f0f0f0); }
...
// #####################################################################
// ## Datepicker Component (Final Version)
// #####################################################################
export type DateInput = Date | string | { toDate: () => Date; _isAMomentObject?: boolean; $d?: Date };
export interface DateRange { [key: string]: [DateInput, DateInput]; }
...
@Component({
    ...
    <div class="datepicker-container">
      @if (showRanges && rangesArray.length > 0 && mode === 'range') {
        <div class="ranges-container">
          <ul>
            @for (range of rangesArray; track range.key) { <li (click)="selectRange(range.value)">{{ range.key }}</li> }
          </ul>
        </div>
      }
      <div class="calendar-container">
        <div class="header">
          <div class="month-year-selects">
            <app-custom-select class="month-select" [options]="monthOptions" [(value)]="currentMonth"></app-custom-select>
            <app-custom-select class="year-select" [options]="yearOptions" [(value)]="currentYear"></app-custom-select>
          </div>
          <div class="nav-buttons">
            <button type="button" class="nav-button" (click)="changeMonth(-1)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="48" d="M328 112L184 256l144 144"/></svg></button>
            <button type="button" class="nav-button" (click)="changeMonth(1)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="48" d="M184 112l144 144-144 144"/></svg></button>
          </div>
        </div>
        <div class="days-grid-wrapper" [@slideInOut]="animationState">
          <div class="days-grid">
            @for (day of weekDays; track day) { <div class="day-name">{{ day }}</div> }
            @for (day of daysInMonth; track $index) {
              <div class="day-cell"
                ...
                @if (day) { <div class="day-number">{{ day | date : 'd' }}</div> }
              </div>
            }
          </div>
        </div>
  ...
  styles: [`
    ...
    .datepicker-container { display: flex; }
    .calendar-container {
    ...
    .month-year-selects app-custom-select.month-select { --custom-select-width: 120px; }
    .month-year-selects app-custom-select.year-select { --custom-select-width: 90px; }
    ...
    .nav-button:hover { background-color: var(--datepicker-hover-background); }
    .nav-button svg { width: 18px; height: 18px; }
    .days-grid-wrapper { position: relative; }
    .days-grid { display: grid; grid-template-columns: repeat(7, 1fr); text-align: center; gap: 0; }
    .day-name { font-weight: 600; font-size: 0.8rem; color: var(--datepicker-subtle-text-color); padding: 8px 0; }
    .day-cell { position: relative; height: 38px; display: flex; justify-content: center; align-items: center; cursor: pointer; border-radius: 0; }
    .day-number { 
      width: 36px; height: 36px; display: flex; justify-content: center; align-items: center; 
      border-radius: 50%; color: var(--datepicker-text-color); 
    ...
    .day-cell.start-date { border-top-left-radius: 50px; border-bottom-left-radius: 50px; }
    .day-cell.end-date { border-top-right-radius: 50px; border-bottom-right-radius: 50px; }
    .day-cell.start-date.end-date { border-radius: 50px; }
    .day-cell.disabled {
      background-color: transparent !important; color: #4b5563;
      cursor: not-allowed; pointer-events: none; opacity: 0.5;
    }
    .day-cell.today .day-number { border: 1px solid var(--datepicker-primary-color); }
    .ranges-container { 
      width: 180px; padding: 16px; 
      border-right: 1px solid var(--datepicker-border-color); 
      background: var(--datepicker-background);
    }
    .ranges-container ul { list-style: none; padding: 0; margin: 0; }
    .ranges-container li { padding: 10px; margin-bottom: 8px; border-radius: 6px; cursor: pointer; color: var(--datepicker-text-color); }
    .ranges-container li:hover { background-color: var(--datepicker-hover-background); }
  `],
  animations: [
    trigger('slideInOut', [
      transition('* => next', [
...
export class NgxsmkDatepickerComponent implements OnInit, OnChanges {
  ...
  @Input() locale: string = navigator.language;
  ...
  public onDateClick(day: Date | null): void {
    ...
    if (this.mode === 'single') {
      ...
      if (!this.startDate || (this.startDate && this.endDate)) {
        this.startDate = day; this.endDate = null;
  ...
  public generateCalendar(): void {
    ...
    for (let i = 0; i < emptyCellCount; i++) { this.daysInMonth.push(null); }
